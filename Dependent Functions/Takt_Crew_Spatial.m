% Constants
function [result,model]=Takt_Crew_Spatial(k,n,C_c,C_o,C_t,a,M_col,M_r,Q)
...
model.quadcon = repmat(struct('Qc', [], 'q', [], 'rhs', [], 'sense', []), k, 1);
...
% Constraints
row = 0;
% [row_idx, col_idx, vals] = find(Q);  % Extract nonzero indices & values
...
for i = 1:k
    ...
    Qc = sparse(total_vars, total_vars); % Initialize sparse matrix
    Qc(b_row_idx, b_row_idx) = Q; % Assign the correct block for this row    
    % Qc = sparse(b_row_idx(row_idx), b_row_idx(col_idx), vals, num_b_vars, num_b_vars);

    % Linear part (sum(b_r(i,:))) 
    % q = sparse(b_row_idx, 1, -1, num_b_vars, 1);  % Place -1 at the correct locations
    q = sparse(total_vars, 1);
    q(b_row_idx) = 1; % Subtract sum(b_r(i,:))
    
    % Define quadratic constraint
    model.quadcon(i).Qc = Qc;  % Quadratic term
    model.quadcon(i).q = q;    % Linear term
    model.quadcon(i).rhs = 0;  % Right-hand side (equality)
    model.quadcon(i).sense = '='; % Equality constraint
